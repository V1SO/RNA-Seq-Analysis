...
res <- results(dds,
               contrast=c("Treatment", "Plasma24hr", "Control"),
               alpha = 0.05,
               pAdjustMethod = "BH",
...
pdf(file="LabelledDifferentiallyExpressed2ControlvsPlasma24.pdf",height = 12,width = 12)
genes <- res.sig.24hr
...
dev.off()
write.csv(as.data.frame(res.sig.24hr), file = "Controlvs24hr.csv")