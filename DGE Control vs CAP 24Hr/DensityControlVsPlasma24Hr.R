...
#load("~/tyson/VikasDifferentialExpressionControlvs24hrPlasma/Vikas.Rdata")
setwd("~/tyson/VikasDifferentialExpressionControlvs24hrPlasma")
allDiff <- res.sig.24hr
...
condition=as.character(c(condition, rep(c("Plasma 24 hr"), times=4)))
plotList <- c()
log2.counts=log2.counts[,c(1:4,9:12)]
for (x in 1:length(GeneNames)) {
  vals=as.numeric(log2.counts[GeneNames[x], ])
  #vals=vals[c(1:4,9:12)]
  myDF=as.tibble(cbind(as.character(condition), as.numeric(vals)))
  colnames(myDF)=c("Condition", "Value")
  name=paste(FriendlyName[x], "Normalized Transcript Counts - Plasma 24 hr vs Control", "Log 2 Fold Change =",
...
combined=plot_grid(plot1, plot2,plot3,plot4,plot5,plot6,plot7,plot8,plot9,plot10, plot11, plot12, plot13, nrow=7, ncol=2)
